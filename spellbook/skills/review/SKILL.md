...
Orquestra revisão de código despachando subagentes `spellbook:reviewer` (checagem técnica + review) e `spellbook:fixer` (correções cirúrgicas), com verificação pós-correção.
...
digraph review_flow {
    ...
    reviewer [label="Despachar spellbook:reviewer\n(./reviewer-prompt.md)"];
    issues [label="Issues encontrados?" shape=diamond];
    fixer [label="Despachar spellbook:fixer\n(./fixer-prompt.md)"];
    ...
    fixer2 [label="Re-despachar spellbook:fixer\ncom novos issues"];
...
Preencha o template `./reviewer-prompt.md` e despache o agente `spellbook:reviewer`:
...
Preencha o template `./fixer-prompt.md` e despache o agente `spellbook:fixer`:
...
Toda revisão DEVE ser feita pelo `spellbook:reviewer`. Toda correção DEVE ser feita pelo `spellbook:fixer`. Você é orquestrador, não executor.
