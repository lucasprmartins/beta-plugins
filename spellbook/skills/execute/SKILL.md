...
Executa planos de implementação despachando subagentes `spellbook:maker` em fases paralelas, agrupadas por dependências. Cada fase aguarda conclusão total antes da próxima.
...
- Arquivo de plano em `.spellbook/plans/` gerado pela skill `/plan`
...
Ao despachar cada maker, usar o agente `spellbook:maker` com o template `./maker-prompt.md`, preenchendo:
