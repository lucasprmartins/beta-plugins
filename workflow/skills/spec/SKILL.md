...
model: sonnet
user-invocable: true
argument-hint: [ideia/tarefa/projeto para o spec]
---
...
**Princípio fundamental:** A especificação descreve O QUE construir, nunca COMO implementar. Código, SQL, schemas e estrutura de arquivos pertencem à skill `/roadmap`.
...
**Anunciar no início:** "Estou usando a skill `spec` para refinar sua ideia em uma especificação. Vou fazer perguntas uma de cada vez e validar cada etapa com você."
...
**Não usar para:** Tarefas com requisitos já claros (use `/roadmap`), bugs, ou perguntas sobre código existente.
...
- Se o usuário pedir implementação, redirecione **imediatamente e explicitamente** na mesma resposta: mencione que código/schema/estrutura pertencem à skill `/roadmap` e use `AskUserQuestion` para confirmar se deseja seguir com o spec ou mudar para `/roadmap`
- Não adie o redirecionamento — não espere o usuário "insistir" para redirecionar
- Redirecionamento "implícito" (mencionar /roadmap de passagem) NÃO conta. Seja direto e claro
- Redirecionar NÃO é patronizar — é proteger o escopo do spec
...
Após ~8 interações, comece a convergir. Não prolongue o spec indefinidamente.
...
| "O usuário pediu código, devo atender" | O brainstorm define O QUE. Código é COMO. Sugira `/roadmap` para implementação. |
| "Vou redirecionar para /roadmap quando ele insistir" | Redirecione AGORA, não depois. Adiar o redirecionamento é aceitar a violação temporariamente. |
| "Mencionei /roadmap implicitamente, já conta" | Implícito não conta. Seja explícito e direto sobre o redirecionamento. |
...
- "Vou redirecionar para /roadmap depois, quando ele insistir"
- "Já mencionei /roadmap implicitamente"
...
> **Para Claude:** Este documento deve ser usado como entrada para a skill `workflow:roadmap`.
...
Após salvar a especificação, informe o caminho do arquivo salvo, pergunte se o usuário precisa de alguma alteração e recomende limpar a janela de contexto antes de executar a próxima etapa (`/roadmap`).
