...
**Anunciar no início:** "Estou usando a skill `check` para solicitar uma revisão de código."

## Quando Usar

- Implementação concluída que precisa ser revisada antes de prosseguir
- Após execução de plano via `/build`
...
- **Vindo do pipeline (`/build`):** usar o SHA base global registrado pelo execute
