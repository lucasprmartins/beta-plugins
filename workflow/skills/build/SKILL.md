...
**Anunciar no início:** "Estou usando a skill `build` para executar um plano de implementação. Vou seguir um processo estruturado para garantir que as tarefas sejam concluídas corretamente."

## Quando Usar

- Plano de implementação pronto com tarefas e dependências definidas
- Arquivo de plano em `.workflow/plans/` gerado pela skill `/roadmap`
...
Após todas as tarefas concluídas, informe o usuário que a implementação foi concluída, pergunte se precisa de alguma alteração e recomende limpar a janela de contexto antes de executar a próxima etapa (`/check`).
